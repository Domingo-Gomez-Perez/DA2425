...
;(fact n) ---->  (lambda (n) ((fact n) n))


;Versión pedida
(((lambda (funcion)
   ((lambda (fact) (funcion (lambda (n) ((fact fact) n))))
    (lambda (fact) (funcion (lambda (n) ((fact fact) n))))))
     (lambda (fact)
       (lambda (n)
         (if (= n 0)
...
#|
Para definir una función llamada fact que recibe como argumento n con el uso de lambda,
debo crear varias funciones lambda que me permiten definir una función
con argumentos fact y n , donde "(fact fact)" se llama a sí mismo y para que esto funcione
 y que sea recursivo,
vuelvo a definir "(lambda (fact) (funcion (lambda (n) ((fact fact) n))))"
