...
; Throughout chapter 1, the only model of computation described
...
; Following the structure ; ((lambda <???> ...) 5), we can use:

((lambda (x)
   ((lambda (f)
      ((lambda (factorial) (factorial factorial x))
       (lambda (factorial n)
         (if (= n 0)
             1
             (* n (factorial factorial (- n 1)))))
       ))
    (lambda (factorial n) (factorial factorial n)))) 5)