...
; For instance, given the definition (define (f x y . z) ⟨body⟩) the
; procedure `f` can be called with two or more arguments. If we evaluate
; (f 1 2 3 4 5 6) then in the body of `f`, `x` will be 1, `y` will be 2, and
; `z` will be the list `(3 4 5 6)`.

; Given the definition   (define (g . w) ⟨body⟩)  the procedure `g` can be called
; with zero or more arguments. If we evaluate      (g 1 2 3 4 5 6)
...
(define (same-parity p . lst)
  (define (aux lst acc)
    (cond
      ((null? lst) (reverse acc))
      ((= (remainder (car lst) 2) (remainder p 2)) (aux (cdr lst) (cons (car lst) acc)))
