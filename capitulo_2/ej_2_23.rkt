...
;Ejemplo de implementación de for-each con el mismo uso


(define (foreach op lista)
  (cond
    ((null? lista) (void))  ; Caso base: si la secuencia es vacía, no hace nada
    (else
     (newline)
     (op (car lista))   ; Aplica la función al primer elemento
     (foreach op (cdr lista)))))  ; Recurre con el resto de la lista

(foreach display (list 57 321 88))
