...
; the special symbol `reset-count`, then `mf` resets the counter to zero. For
; any other input, `mf` returns the result of calling `f` on that input and
...
(s 100)                ; Should return 10
(s 121)                ; Should return 11
(s 'how-many-calls?)   ; Should return 2

(s 'reset-count)
(s 'how-many-calls?)   ; Should return 0