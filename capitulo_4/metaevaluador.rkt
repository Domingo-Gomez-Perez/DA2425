...
(define (lookup-in-enviroment v environ)
  (hash-ref environ v (lambda() (error "Variable no definida: "v)))
  )
...
(define (seval-define exp environ)
  (let ((name (define-name exp))
    ...
    (define-in-enviroment! name (seval value environ) environ)
    )
  )
...
(define (seval-begin exp environ)
  (define exprs (begin-expressions exp))
  (cond
    [(null? exprs) #f] 
    ...
    [else
     (begin
       ...
       (seval-begin (cons 'begin (cdr exprs)) environ))]))
...
(define (make-procedure params body environ)
  (lambda (args)
         (let ((new-environ(extend-environment params args environ)))
...
(define (extend-environment params args parent-environ)
  (let extend-helper ((params params)
                      (args (if (list? args) args (list args))) ;Se necesita que args sea siempre una lista, si no da problemas
                      (display args)
                      (new-environ (make-hash)))
    (cond
      [(and (null? params) (null? args))
       (begin
         (hash-set! new-environ 'parent parent-environ)
         new-environ)]
      [(or (null? params) (null? args))
       (error "Error de aridad")]
      [else
       (begin
          
         (hash-set! new-environ (car params) (car args))
         (extend-helper (cdr params) (cdr args) new-environ parent-environ))])))
...
(define-in-enviroment! 'foo 123 environ)
...
;(check-equal? (seval '(square 4) environ) 16 "square failed") ;Hay un error que es que toma el valor de x de las funciones anteriores de 42
